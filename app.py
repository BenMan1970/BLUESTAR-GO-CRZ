...
class RateLimiter:
    ...
    min_interval: float = 0.12  # 120ms minimum entre requêtes
    max_retries: int = 3
...
class TradingParams:
    ...
    min_score_threshold: int = 50  # ✅ Nouveau: score minimum
...
class RiskConfig:
    ...
    min_win_rate: float = 0.45  # ✅ Nouveau: win rate minimum
    max_win_rate: float = 0.75  # ✅ Nouveau: win rate maximum
...
def get_oanda_client():
    try:
        ...
        if not token:
            raise ValueError("OANDA_ACCESS_TOKEN manquant dans secrets")
...
def run_scan(pairs: List[str], tfs: List[str], mode_live: bool, 
    ...
    """Scan multi-threadé avec timeout et stats"""
    
    start_time = time.time()
    signals = []
    stats = ScanStats(total_pairs=len(pairs) * len(tfs))
    
    # ✅ FIX: ThreadPool avec timeout
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(futures, timeout=60):  # 60s timeout global
            pair, tf = futures[future]
            try:
                result = future.result(timeout=10)  # 10s timeout par paire
                if result:
                    signals.append(result)
                    stats.signals_found += 1
                stats.successful_scans += 1
            except TimeoutError:
                error_msg = f"{pair} {tf}: Timeout (>10s)"
    ...
    logger.info(f"✅ Scan terminé: {stats.signals_found} signaux en {stats.scan_duration:.1f}s")
...
def generate_pdf(signals: List[Signal]) -> bytes:
    ...
    elements.append(Paragraph("<font size=16 color=#00ff88><b>BlueStar v2.4 - Signaux Institutionnels</b></font>", 
                             styles["Title"]))
    elements.append(Spacer(1, 8*mm))
    
    now = datetime.now(TUNIS_TZ).strftime('%d/%m/%Y %H:%M:%S')
    elements.append(Paragraph(f"<font size=10 color=#a0a0c0>Généré le {now} (Tunis)</font>", 
                             styles["Normal"]))
    elements.append(Spacer(1, 10*mm))
    
    data = [["Heure", "Paire", "TF", "Qualité", "Action", "Entry", "SL", "TP", "Score", 
             "R:R", "Taille", "Risque $", "ADX", "RSI", "Tendance", "Flip", "Live"]]
...
def main():
    ...
    st.markdown("""<div style='text-align: center; color: #666; font-size: 0.7rem; padding: 15px;'>
        ...
        Optimizations: Rate Limiter ✅ | Retry Logic ✅ | Timeout Handling ✅ | 
        HMA Strict Validation ✅ | Score >= 50 ✅
    </div>""", unsafe_allow_html=True)

if __name__ == "__main__":
    main()error(f"❌ OANDA Client Error: {e}")
        ...
        def wrapper(*args, **kwargs):
            for attempt in range(max_attempts):
                try:
                    ...
                    logger.warning(f"⚠️ OANDA API Error (attempt {attempt + 1}/{max_attempts}): {e}")
                    if attempt < max_attempts - 1:
                        sleep_time = rate_limiter.backoff(attempt)
                        time.sleep(sleep_time)
                    else:
...
def _cache_wrapper(cache_key: str, fetch_func, *args, **kwargs):
    """Wrapper de cache pour contourner les limitations de st.cache_data"""
    return fetch_func(*args, **kwargs)

@retry_on_failure(max_attempts=3)
def fetch_candles_raw(pair: str, tf: str, count: int) -> pd.DataFrame:
    """Récupération brute des bougies sans cache"""
    ...
    try:
        ...
        logger.debug(f"✅ {pair} {tf}: {len(df)} candles | Last complete: {df.iloc[-1]['complete'] if not df.empty else 'N/A'}")
        return df
...
def get_candles(pair: str, tf: str, count: int = 300, use_cache: bool = True) -> pd.DataFrame:
    """Récupération des bougies avec cache intelligent"""
    if not use_cache:
        return fetch_candles_raw(pair, tf, count)
    
    cache_key = get_cache_key(pair, tf, count)
    
    # TTL adaptatif selon timeframe
    ttl_map = {"H1": 30, "H4": 120, "D1": 300}
    ttl = ttl_map.get(tf, 30)
    
    return _cache_wrapper(cache_key, fetch_candles_raw, pair, tf, count)

# ==================== INDICATEURS OPTIMISÉS ====================
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """Calcul des indicateurs avec validation stricte"""
    if len(df) < 50:
        logger.warning("⚠️ Pas assez de données pour calculer les indicateurs")
        return df
    ...
    # WMA avec validation stricte
    def wma(series, length):
        if len(series) < length:
            return pd.Series([np.nan] * len(series), index=series.index)
        weights = np.arange(1, length + 1)
        result = series.rolling(length, min_periods=length).apply(
            lambda x: np.dot(x, weights) / weights.sum() if len(x) == length else np.nan,
            raw=True
        )
        return result

    # HMA (Hull Moving Average) - STRICTE
    try:
    ...
    try:
        ...
        logger.error(f"❌ Erreur calcul UT Bot: {e}")
        df['ut_state'] = 0
...
def get_trend_alignment(pair: str, signal_tf: str) -> str:
    ...
    try:
        ...
        logger.error(f"❌ Erreur cascade {pair} {signal_tf}: {e}")
        return "Neutral"
...
class RiskManager:
    ...
    def calculate_position_size(self, signal: Signal, win_rate: float = 0.58) -> float:
        """Calcul de la taille de position avec Kelly Criterion validé"""
        # ✅ FIX: Validation du win_rate
        ...
        if kelly <= 0:
            logger.warning(f"⚠️ Kelly négatif ou nul pour {signal.pair}: {kelly}")
            return 0.0
        
        pip_risk = abs(signal.entry_price - signal.stop_loss)
        if pip_risk <= 0:
            logger.warning(f"⚠️ Pip risk invalide pour {signal.pair}: {pip_risk}")
            return 0.0
...
def analyze_pair(pair: str, tf: str, mode_live: bool, risk_manager: RiskManager, params: TradingParams) -> Optional[Signal]:
    """Analyse d'une paire avec gestion optimisée"""
    
    try:
        df = get_candles(pair, tf, 300, use_cache=True)
        if len(df) < 100:
        ...
        # ✅ FIX: Validation stricte des indicateurs
        required_fields = ['hma', 'rsi', 'adx', 'atr_val', 'hma_up', 'ut_state']
        for field in required_fields:
            if pd.isna(last[field]):
                logger.debug(f"⚠️ {pair} {tf}: Indicateur {field} invalide à idx={idx}")
                return None
        
        if pd.isna(prev['hma_up']) or (prev2 is not None and pd.isna(prev2['hma_up'])):
            logger.debug(f"⚠️ {pair} {tf}: HMA historique invalide")
            return None
