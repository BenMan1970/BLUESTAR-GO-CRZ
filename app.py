...
✅ ATR Percentile calcul (vrai percentile 0-100)
✅ HMA flip detection (gestion NaN)
...
<style>
    ...
    /* MODIFICATION 2: Suppression du quadrillage */
    .stDataFrame {font-size: 0.75rem !important;}
    .stDataFrame [data-testid="stDataFrame"] div[role="grid"] div {
...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        # ATR Percentile CORRECT (vrai percentile 0-100)
...
def get_trend_alignment(pair: str, signal_tf: str) -> Tuple[str, float]:
    ...
    # MODIFICATION 3: Mapping cascade incluant M15
...
def generate_pdf(signals: List[Signal]) -> bytes:
    ...
    elements.append(Paragraph("<font size=16 color=#00ff88><b>BlueStar v3.0</b></font>", styles["Title"]))
...
def main():
    ...
    with col_title:
        # MODIFICATION 1: Suppression du mot "FIXED"
        st.markdown("# BlueStar Enhanced v3.0")
        st.markdown('<span class="institutional-badge">INSTITUTIONAL</span><span class="v30-badge">v3.0 ADVANCED</span><span class="fixed-badge">READY</span>', 
                   unsafe_allow_html=True)
    ...
    if scan_btn:
        with st.spinner("🔍 Scanning markets with v3.0 engine..."):
            ...
            # MODIFICATION 3: Ajout M15 dans le scan
            signals, stats = run_scan(PAIRS_DEFAULT, ["M15", "H1", "H4", "D1"], is_live, 
                                     risk_manager, params)
        ...
        if signals:
            ...
            st.markdown("---")
            # MODIFICATION 3: Ajout Colonne M15
            col_m15, col_h1, col_h4, col_d1 = st.columns(4)
            
            for col, tf in zip([col_m15, col_h1, col_h4, col_d1], ["M15", "H1", "H4", "D1"]):
                with col:
    ...
    st.markdown("""<div style='text-align: center; color: #666; font-size: 0.7rem; padding: 15px;'>
        ...
        🔧 ATR Percentile ✅ | HMA Flip ✅ | Optimized Defaults ✅ | Debug Logging ✅
    </div>""", unsafe_allow_html=True)
