# BlueStar Cascade - Refactored v2.7 (optimised)
...
_MIN_REQUEST_INTERVAL = 0.20  # s, pour éviter d'écraser l'API
...
@dataclass
class RiskConfig:
    max_risk_per_trade: float = 0.01  # proportion du capital
...
# ====== API client ======
@st.cache_resource
def get_oanda_client() -> Optional[API]:
    token = st.secrets.get("OANDA_ACCESS_TOKEN")
    if not token:
        logger.error("OANDA_ACCESS_TOKEN missing in secrets.toml")
        return None
...
# ====== UTIL / DATA FETCH ======
def _throttle():
    """Respecte l'intervalle minimal entre requêtes."""
...
def get_candles(pair: str, tf: str, count: int = 300) -> pd.DataFrame:
    """
    Récupère les candles depuis OANDA. Retourne DataFrame vide en cas d'erreur.
    """
    gran = GRANULARITY_MAP.get(tf)
    if not gran:
        logger.warning("Timeframe inconnu: %s", tf)
        return pd.DataFrame()
    ...
    try:
        ...
        logger.exception("Erreur récupération candles %s: %s", pair, e)
        return pd.DataFrame()
...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """
    Ajoute colonnes: hma, hma_up, rsi, adx, atr_val, ut_state
    Doit être appelé sur une copie ou DataFrame source.
    """
    ...
    # WMA helper (rolling apply, assez rapide en pandas)
    ...
    hma_series = wma(2 * wma_half - wma_full, int(np.sqrt(n)))
    df["hma"] = hma_series
    ...
    # Trailing stop (UT-style) vectorized-ish
    # On construit un tableau numpy pour faire la boucle en local (moins d'overhead)
    close_vals = close.to_numpy()
    atr_loss = (2.0 * atr14).fillna(method="bfill").to_numpy()
    nrows = len(df)
    trailing = np.zeros(nrows, dtype=float)
    # init trailing to first candle price (or close - loss)
    trailing[0] = close_vals[0] - atr_loss[0]
    for i in range(1, nrows):
        prev_stop = trailing[i - 1]
        cur = close_vals[i]
        loss = atr_loss[i]
        # logique identique à l'originale, exécutée en boucle locale (bien plus rapide car arrays en local)
...
def get_trend_alignment(pair: str, signal_tf: str) -> str:
    ...
    if not higher_tf:
        return "Neutral"
    df = get_candles(pair, higher_tf, count=120)
...
def analyze_pair(pair: str, tf: str, mode_live: bool, risk_manager, params: TradingParams) -> Optional[Signal]:
    ...
    # Choix de l'index (live vs confirmed)
    if mode_live:
        idx = -1
    else:
...
class RiskManager:
    ...
    def calculate_position_size(self, signal: Signal) -> float:
        """
        Retourne taille (units). Strategy:
         - calcule d'abord la taille basée sur max_risk_per_trade (plus simple et plus sûr)
         - si kelly_fraction > 0, combine une correction
        """
        pip_risk = abs(signal.entry_price - signal.stop_loss)
        if pip_risk <= 0 or signal.entry_price <= 0:
            return 0.0

        # risk by account % (safety first)
        risk_amount = self.balance * min(max(self.config.max_risk_per_trade, 0.0001), 1.0)
        size_by_risk = risk_amount / pip_risk

        # optional Kelly adjustment (conservative)
        win_rate = 0.55
        ...
        # Combined approach: weighted mean between pure risk sizing and kelly adjustment
    ...
    for pair in pairs:
        for tf in tfs:
            ...
            try:
                ...
                logger.exception("Erreur analyse pair %s %s", pair, tf)
                continue
    progress.empty()
    return signals
...
def generate_pdf(signals: List[Signal]) -> bytes:
    ...
    elements.append(Paragraph(f"Généré le: {now} (TUNIS)", styles["Normal"]))
...
# ====== INTERFACE (inchangée logiquement) ======
def main():
    # UI custom CSS omitted for brevity - tu peux réinjecter ton style HTML si tu veux.
