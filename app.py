    ...
    try:
        ...
        if not df.empty:
            ...
            if validate_price_data(df, pair):
                ...
                logger.warning(f"{pair} {tf}: Données suspectes détectées")
...
def analyze_pair(pair: str, tf: str, mode_live: bool, risk_manager: RiskManager,
    ...
    if params.cascade_required:
        if action == "BUY" and higher_trend != "Bullish":
            logger.info(f"Cascade reject: {pair} {tf} BUY vs {higher_trend}")
            return None
        if action == "SELL" and higher_trend != "Bearish":
            logger.info(f"Cascade reject: {pair} {tf} SELL vs {higher_trend}")
            return None
    ...
    if rr_ratio < params.min_rr_ratio:
        logger.info(f"R:R reject: {pair} {tf} ({rr_ratio:.2f} < {params.min_rr_ratio})")
        return None
...
def generate_pdf(signals: List[Signal]) -> bytes:
    ...
    # Titre
    title = Paragraph("<font size=16 color=#00ff88><b>BlueStar Cascade - Signaux Institutionnels</b></font>", styles["Title"])
    elements.append(title)
    elements.append(Spacer(1, 8*mm))
    
    now_tunis = datetime.now(pytz.timezone('Africa/Tunis')).strftime('%d/%m/%Y %H:%M:%S')
    subtitle = Paragraph(f"<font size=10 color=#a0a0c0>Généré le {now_tunis} (Tunis)</font>", styles["Normal"])
    elements.append(subtitle)
    elements.append(Spacer(1, 10*mm))
    
    # Tableau
    data = [["Heure", "Paire", "TF", "Qualité", "Action", "Entry", "SL", "TP", "Score", "R:R", "Taille", "Risque $", "ADX", "RSI", "Tendance Sup.", "Fresh Flip", "Live"]]
    ...
    for s in sorted(signals, key=lambda x: (x.score, x.timestamp), reverse=True):
        data.append([
            ...
            str(s.rsi),
            s.higher_tf_trend,
            "Oui" if s.is_fresh_flip else "Non",
            "Oui" if s.is_live else "Non"
        ])
    
    table = Table(data, colWidths=[15*mm, 20*mm, 10*mm, 20*mm, 15*mm, 18*mm, 18*mm, 18*mm, 12*mm, 12*mm, 15*mm, 15*mm, 10*mm, 10*mm, 22*mm, 18*mm, 12*mm])
    ...
    if scan_btn:
        ...
        if signals:
            ...
            with col_dl3:
                ...
                st.download_button(
                    label="Télécharger Signaux (PDF)",
                    data=pdf_data,
                    file_name=f"bluestar_signals_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
                ...
                with col:
                    ...
                    if tf_signals:
                        ...
                        df_clean = pd.DataFrame([{
                            ...
                            "Action": f"{'BUY' if s.action == 'BUY' else 'SELL'}{'Live' if s.is_live else ''}{'Fresh' if s.is_fresh_flip else ''}",
                            "Score": s.score,
                            "Entry": f"{s.entry_price:.5f}",
                            "SL": f"{s.stop_loss:.5f}",
                            "TP": f"{s.take_profit:.5f}",
