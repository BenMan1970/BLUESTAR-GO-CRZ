...
Améliorations : Logging, Paramètres configurables, Validation des données + Export PDF
"""
...
def get_oanda_client():
    try:
        ...
        st.error("OANDA Token manquant ou invalide")
        st.stop()
...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['hma_up'] = df['hma'] > df['hma'].shift(1)   # ← CORRIGÉ ICI
...
# ==================== CASCADE + RISK + SIGNAL + SCAN ====================
# (tout le reste est exactement comme avant, juste propre et fonctionnel)
...
def get_trend_alignment(pair: str, signal_tf: str) -> str:
    ...
    def wma(series, length):
        weights = np.arange(1, length + 1)
        return series.rolling(length).apply(lambda x: np.dot(x, weights) / weights.sum(), raw=True)
    hma_half = wma(close, 10)
    hma_full = wma(close, 20)
    hma = wma(2 * hma_half - hma_full, int(np.sqrt(20)))
    if close.iloc[-1] > ema50 and hma.iloc[-1] > hma.iloc[-2]:
        return "Bullish"
    elif close.iloc[-1] < ema50 and hma.iloc[-1] < hma.iloc[-2]:
        return "Bearish"
    return "Neutral"
...
class RiskManager:
    ...
    def calculate_position_size(self, signal: Signal) -> float:
        ...
        size = (self.balance * kelly) / pip_risk if pip_risk > 0 else 0
        return round(size, 2)
...
def analyze_pair(pair: str, tf: str, mode_live: bool, risk_manager: RiskManager, params: TradingParams) -> Optional[Signal]:
    ...
    last = df.iloc[idx]; prev = df.iloc[idx-1]; prev2 = df.iloc[idx-2]
    hma_flip_green = last.hma_up and not prev.hma_up
    hma_flip_red = not last.hma_up and prev.hma_up
    raw_buy = (hma_flip_green or (last.hma_up and not prev2.hma_up)) and last.rsi > 50 and last.ut_state == 1
    raw_sell = (hma_flip_red or (not last.hma_up and prev2.hma_up)) and last.rsi < 50 and last.ut_state == -1
    ...
    if hma_flip_green or hma_flip_red: score += 15
...
def run_scan(pairs, tfs, mode_live, risk_manager, params):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(analyze_pair, p, tf, mode_live, risk_manager, params) for p in pairs for tf in tfs]
...
def generate_pdf(signals: List[Signal]) -> bytes:
    ...
    data = [["Heure", "Paire", "TF", "Qualité", "Action", "Entry", "SL", "TP", "Score", "R:R", "Taille", "Risque $", "ADX", "RSI", "Tendance Sup.", "Fresh", "Live"]]
    for s in sorted(signals, key=lambda x: (x.score, x.timestamp), reverse=True):
        data.append([
            s.timestamp.strftime("%H:%M"), s.pair.replace("_", "/"), s.timeframe, s.quality.value, s.action,
            f"{s.entry_price:.5f}", f"{s.stop_loss:.5f}", f"{s.take_profit:.5f}", str(s.score), f"{s.risk_reward:.1f}",
            f"{s.position_size:.2f}", f"{s.risk_amount:.0f}", str(s.adx), str(s.rsi), s.higher_tf_trend,
            "Oui" if s.is_fresh_flip else "Non", "Oui" if s.is_live else "Non"
        ])
    table = Table(data, colWidths=[14*mm,18*mm,10*mm,20*mm,14*mm,18*mm,18*mm,18*mm,10*mm,10*mm,14*mm,14*mm,10*mm,10*mm,20*mm,14*mm,12*mm])
...
def main():
    ...
    if scan_btn:
        with st.spinner("Scanning markets..."):
            params = TradingParams(atr_sl_multiplier=atr_sl, atr_tp_multiplier=atr_tp, min_rr_ratio=min_rr, cascade_required=cascade_req)
            risk_manager = RiskManager(RiskConfig(max_risk_per_trade=max_risk, max_portfolio_risk=max_portfolio), balance)
            signals = run_scan(PAIRS_DEFAULT, ["H1", "H4", "D1"], is_live, risk_manager, params)
        ...
        if signals:
            ...
            m3.metric("Avg Score", f"{np.mean([s.score for s in signals]):.0f}")
            m4.metric("Exposure", f"${sum(s.risk_amount for s in signals):.0f}")
            m5.metric("Avg R:R", f"{np.mean([s.risk_reward for s in signals]):.1f}:1")
            ...
            with dl1:
                df_csv = pd.DataFrame([{
                    ...
                    "Quality": s.quality.value, "Action": s.action, "Entry": s.entry_price, "SL": s.stop_loss, "TP": s.take_profit,
                    "Score": s.score, "R:R": s.risk_reward, "Size": s.position_size, "Risk $": s.risk_amount,
                    "ADX": s.adx, "RSI": s.rsi, "Higher TF": s.higher_tf_trend, "Fresh Flip": s.is_fresh_flip, "Live": s.is_live
                } for s in signals])
                st.download_button("Télécharger CSV", df_csv.to_csv(index=False).encode(), f"bluestar_{datetime.now().strftime('%Y%m%d_%H%M')}.csv", "text/csv", use_container_width=True)
            # PDF
            with dl3:
                pdf = generate_pdf(signals)
                st.download_button("Télécharger PDF", pdf, f"bluestar_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf", "application/pdf", use_container_width=True)
                ...
                with col:
                    ...
                    if tf_sig:
                        ...
                        df_disp = pd.DataFrame([{
                            ...
                            "Qualité": s.quality.value, "Action": f"{'BUY' if s.action=='BUY' else 'SELL'}{'Live' if s.is_live else ''}{'Fresh' if s.is_fresh_flip else ''}",
                            "Score": s.score, "Entry": f"{s.entry_price:.5f}", "SL": f"{s.stop_loss:.5f}", "TP": f"{s.take_profit:.5f}",
                            "R:R": f"{s.risk_reward:.1f}", "Taille": f"{s.position_size:.2f}", "Risque": f"${s.risk_amount:.0f}",
                            "ADX": s.adx, "RSI": s.rsi
                        } for s in tf_sig])
    ...
    st.markdown("---")
    st.markdown("<div style='text-align: center; color: #666; font-size: 0.7rem; padding: 15px;'><b>BlueStar Cascade Enhanced</b> | v2.1 - PDF Export + Bugfix</div>", unsafe_allow_html=True)
